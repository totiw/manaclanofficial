...
const Tabel = lazy(() => import("../../components/MonsterExp/Table"));
const Monster = () => {
  const [monsterExp, setMonsterExp] = useState([]);
  const getMonsterExp = async () => {
    try {
      const response = await axios.get("/get/monsterexp");
      setMonsterExp(response.data);
    } catch (err) {
